...
VALUES((SELECT id FROM "user" WHERE username ilike '%kubo%'), 
	   '解雇されました','私の契約は終了されました...')
...
--Ejercicio 5:
INSERT INTO "user"(username, password)
VALUES ('Leo Messi', (SELECT password FROM "user" WHERE username ilike '%kubo%'))
...
body = ''
WHERE author_id in (SELECT id from "user" WHERE username ilike '%mitoma%')

--Actualizar todos los posts de un usuario (seleccionado con subquery
--por nombre) y poner el cuerpo del post en texto vacío ("").

--Ejercicio 7:
DELETE FROM "post" WHERE author_id in (SELECT id FROM "user" WHERE username ilike '%mitoma%');
DELETE FROM "user" WHERE username ilike '%mitoma%';
